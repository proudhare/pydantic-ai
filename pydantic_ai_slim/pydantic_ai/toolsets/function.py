...
class FunctionToolset(AbstractToolset[AgentDepsT]):
    ...
    def add_tool(self, tool: Tool[AgentDepsT]) -> None:
        ...
        """
        if self._get_tools_in_progress > 0:
            raise UserError(
                'Cannot add tools during get_tools() execution. '
                'Tool registration from prepare callbacks is not supported. '
                'Use DynamicToolset, prepare_tools, or DeferredLoadingToolset for dynamic tool registration.'
            )
