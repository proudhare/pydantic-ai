    ...
    def _request(
        ...
        if isinstance(output_wrapper, _WrappedTextOutput):
            if (response_text := output_wrapper.value) is None:
                ...
                if output:
                    return ModelResponse(
                        parts=[TextPart(pydantic_core.to_json(output).decode())], model_name=self._model_name, provider_name=self._system
                    )
                else:
                    return ModelResponse(parts=[TextPart('success (no tool calls)')], model_name=self._model_name, provider_name=self._system)
            else:
                return ModelResponse(parts=[TextPart(response_text)], model_name=self._model_name, provider_name=self._system)
        else:
